 ...
 * @file		debugTask.h
 *
 * @brief		«аголовок отладочного модул€
 *
 * @version		v1.0
 * @date		05.09.2013
 * @author		Mike Smith
...
//*			 онстанты
//*-----------------------------------------------------------------------------------------------
#define READ_SIZE				512			// размер буфера дл€ чтени€ из файла
#define TEST_TASK_STK_SIZE		220			// размер стека задачи 220
#define FILE_PATH_MAX_LENGTH	34			// размер строки дл€ пути файла "0:/dirname_/dirname_/filename.hcd" - 33 символа
#define DIR_PATH_MAX_LENGTH		24			// размер строки дл€ пути рабочей папки "0:/dirname_/dirname_/" - 21 символ
#define FILE_NAME_MAX_LENGTH	13			// максимальна€ длина имени файла

#define MainLoop_Maximum_Wait_ms		500

#define SERVO_DEAD_BAND			100			//период вызова таймера сервов

#define SD_Recheck_Countdown_REVOLVE 512 //цикл подключени€ карты на гор€чую


//#if defined (FastButtonShortcuts)
/*
 ороткие команды ( байта)
1 байт - сама команда, он же номер ссылки в select case
2 байт - параметр (вкл/выкл; битова€ маска пинов/выводовб которые активировать)
3 и 4 байты - параметр (число)
*/
	#define FC_TOTAL_COMMAND_LIST		0x25	//сколько всего команд есть в списке

//следующие 3 - строго в этом пор€дке и строго в начале
...
//следующие 4 - строго в этом пор€дке и строго после блока команд M98
	#define FC_ENTER_SUBPROGRAM_M89		0x04	//M89 P<>, M89 Q<> P<> - перейти в подпрограмму
	#define FC_EXIT_SUBPROGRAM_M89		0x05	//M89  - без параметра - вернутьс€ в сновную программу
	//следующие 2 - строго в этом пор€дке и строго после блока команд M98
	#define FC_REPEAT_FILE_M47 			0x06	//M47 P<repeat>,instant, также €вл€етс€ границей переключени€ параметра команды с имени файла на число
	#define FC_REPEAT_COUNTS_M47 		0x07	//How many times to repeat the M47, affects the M47 exec

//следующие 2 - строго в этом пор€дке
	#define FC_RESUME_FILE_M24 			0x08	//M24, resume file execution
	#define FC_SUSPEND_FILE_M25 		0x09	//M25, suspend file eecution
//следующие 5 - строго в этом пор€дке
...
//следующие 4 - строго в этом пор€дке
...
//следующие 10 - строго в этом пор€дке
	#define FC_BRIGHTNESS_SET_G27		0x19	//G27 P<>, SI P<> - настройка абсолютной €ркости
	...
	//следующие 3 - строго в этом пор€дке
...
uint32_t ModifyParameter (uint32_t Inp, uint32_t *FS_arg);
