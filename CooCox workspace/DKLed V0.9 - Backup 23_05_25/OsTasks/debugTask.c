...
//0x04+0x08 (0x0С) - куда выдаём ответ (0000 - неопределено, 0100 - Uart1, 1000 - Uart2)
...
//b1 (1) - 0 = читаем число в hex формате, 1 - читаем число в dec формате
//b2 (2) - 0 = нормальная команда, 1 = короткая команда
//b3 (4) - 0 = положительный, 1 = отрицательный
//b4 (8) - 0 - точное значениеб 1 - дельта
//b5 (16) - 0 = нормальный вид команды, 1 = расширеннвй вид команды
...
Start_button_file_in_case_of_NoAnim:
		...
		case FC_REPEAT_FILE_M47:
			if ((*FS_arg == 0) || ((File_Read_Cycle_Count !=1) && (*FS_arg != 1))){ //было (*FS_arg == 0) || ((File_Read_Cycle_Count !=1) && (*FS_arg != 1))
				...
				if (*FS_arg > 0 || File_Read_Cycle_Count > 1){
					if ((File_Read_Cycle_Count == 0) || FS_b8 != 1 ) { //если это по кнопке то FS_b8=0, значит цикл начинаем сначала
...
M24_Execution_Path_Label:
		...
		case FC_SERVO_POS_SET_G0:
			for (a8 = 0; a8<8; a8++){
				if (FS_b8 & (1<<a8)){
					Servo_Pos_Temp[a8] =  ModifyParameter(Servo_Pos_Temp[a8], FS_arg);
					if (Servo_Pos_Temp [a8] > Servo_Resolution) {
						Servo_Pos_Temp [a8] = Servo_Resolution;
					}
				}
			}
		...
		case FC_BRIGHTNESS_SET_G27:
			DS_LED_Brightness = 1 + ModifyParameter(DS_LED_Brightness-1, FS_arg);
		...
		case FC_CHOSEN_FILE_SET_M91:
			ChosenNomberInM98 = ModifyParameter(ChosenNomberInM98, FS_arg);
		...
		case FC_FAST_ANIM_FREQ_SET_G30:
			...
			*FS_arg =  ModifyParameter((SERVO_TICK_FREQ/WS2812_Frame_Period), FS_arg);
...
void TestTask(void)//* pdata)
{
	...
	Next_File_Num = 0;
	Prev_File_Num = 0;
	SD_Volume_Exists |= 0x11; //мы не находили карточку (0x10) + мы проверяем CRC (0x1)
...
void MainLoop(){
	...
	if ((SD_Volume_Exists & 0x10)==0) { //если есть память в слоте (или, по крайней мере, была прошлый раз)
		...
		if (SD_Volume_Exists&0x80) { //возврат из подпрограммы в основную программу
			...
			SD_Volume_Exists &= ~0xC0; //0x80+0x40
		} else if (~SD_Volume_Exists&0x40){//если это основной файл, то сохраняем позицию
...
//						&DS_status, &DS_comm_num, &DS_WSpoint_counter, &DS_RGB_counter, &DS_maxCurrPoints, &DS_Param);
				...
				/*while (USB_Function_flags & 0x10){
					if (CoGetOSTime() > USB_rx_timeout){
						...
						descrResult = 0;
						goto USB_Skip_reading_buffer_and_Abort;
					}
				}*/
				USB_Recieved_bytes--;
...
//				USB_Function_flags |= 0x10;//not full
				descrResult = Descript(WS2812_IO_framedata, USB_Buff1, &USB_RX_Start_pointer, &USB_Recieved_bytes,
										&DS_status, &DS_comm_num, &DS_WSpoint_counter, &DS_RGB_counter, &DS_maxCurrPoints, &DS_Param);
USB_Skip_reading_buffer_and_Abort:
			...
			switch (descrResult) {
				...
				case DS_ANS_REPEAT_FILE: //M2, M47, M47 P<>, M98
					...
					if (AfterLoop_File_Num == 0xFFFF){
						...
						FastCommand(FC_REPEAT_FILE_M47,1,&DS_Param);
#endif
#ifndef FastButtonShortcuts
...
//						numFileForButton[DS_WSpoint_counter]=;
				...
				case DS_ANS_SERVO_SET:
					...
					DS_comm_num = (DS_comm_num & 4) ? FC_SERVO_POS_RAND_G0 : (FC_SERVO_POS_SET_G0+((DS_Flag_Register & 8)>>3)+((DS_Flag_Register & 4)>>2));
...
//						FastCommandForButton[ButtonNomberCarrier]=FC_SERVO_POS_SET_G0+((DS_Flag_Register & 8)>>3)+((DS_Flag_Register & 4)>>2);
				...
				case DS_ANS_SET_LED_TYPE: //M06
					// в DS_Param находится значение типа светодиодов - 0x122812 для WS или 0x236812 для SK, для среднего
					  if ((DS_Param==0x122812) || (DS_Param==0x122812b)){
						  ...
						  //WS2812_IO_High = 0xFF;
						  //WS2812_IO_Low = 0x00;
						  WS2812_Timer_reinit(29,  6, 14); //sk6812 freq
						  goto SET_LED_Type_LedOnly_Parameters;
...
SET_LED_Type_LedOnly_Parameters:
				...
				case DS_ANS_U1T_DONE:
					//только что была завершена отправка на USART1 работает сдвиговый регистр
				...
				//case DS_ANS_U2T_DONE:
					//только что была завершена отправка на USART2 работает сдвиговый регистр
					//надо поднять флаг паузы, если мы ждём окончания вывода
					if (DS_RGB_counter < 2){
				...
				case DS_I2C_GET_FROM://прочитать быйты из I2C
					...
					//Port_send_char(DS_inttochar((I2C_Bytes_to_Recieve>>4)&0x0F),15);
					//Port_send_char(DS_inttochar(I2C_Bytes_to_Recieve&0xF),15);
					start_I2C(1);
					break;
				...
				case DS_ANS_MULTI_SET_PARAM:
					...
					if (DS_comm_num == 0x26) {
							...
							ButtonWaitSetting =  ModifyParameter(ButtonWaitSetting, &DS_Param);
					} else if (DS_comm_num == 0x27){
						ButtonCompleteSetting = ModifyParameter(ButtonCompleteSetting, &DS_Param);
					...
					} else if(DS_comm_num == 0x28 || DS_comm_num == 0x2B || DS_comm_num == 0x16){ //SI P<>=0x28, M91 P<>=0x2B, M46 P<> = 16
...
Link_to_Modify_parameters_DS_comm_num_IS_ACTION:
					...
					if (DS_comm_num == 0x29){//G29 начало первого проигрываемого кадра анимации мультикадра в памяти
						...
						/*if (DS_Flag_Register & 8){
							if (DS_Flag_Register & 4) {
								DS_Param = ((SERVO_TICK_FREQ/WS2812_Frame_Period) > DS_Param) ? ((SERVO_TICK_FREQ/WS2812_Frame_Period) - DS_Param) : 0;
							} else {
								DS_Param = ((SERVO_TICK_FREQ/WS2812_Frame_Period) + DS_Param);
							}
						}/**/
						DS_comm_num = FC_FAST_ANIM_FREQ_SET_G30;
						goto Link_to_Modify_parameters_DS_comm_num_IS_ACTION;
						/*DS_Param =  ModifyParameter((SERVO_TICK_FREQ/WS2812_Frame_Period), &DS_Param);

						if (DS_Param < SERVO_FREQ_MIN) {DS_Param = SERVO_FREQ_MIN;}
						if (DS_Param > SERVO_FREQ_MAX) {DS_Param = SERVO_FREQ_MAX;}//скорость при 16 пикселах
						if (DS_Param > ((SERVO_FREQ_MAX << 4)/WS2812_Frame_Length)) {DS_Param = (SERVO_FREQ_MAX << 4)/WS2812_Frame_Length;}
						WS2812_Frame_Period = SERVO_TICK_FREQ / DS_Param;
						/**/
				...
				case DS_ANS_FILEMODE_ON: //Переход а режим работы с файлами
					if (DS_Param) {
						...
						//result = f_opendir (&dir, FileworksDirpath );
						SD_Volume_Exists |= ((DS_Channel_Select - 1) & 3)<<2;
						...
						//DS_Channel_Select = 4-строго USB, значение b1100, a8 = b11 - 1 = 2, USB
				...
				case DS_ANS_FILEMODE_NEWFILE: //F9 создаём временный файл для записи в него всякого
						...
						if (result == FR_OK){
							//a8=((SD_Volume_Exists>>2) - 1);
							Message ("Temp file created\0",((SD_Volume_Exists>>2) - 1));
							Finish_Fileworks_Message((SD_Volume_Exists>>2) - 1);
						} else {
							Fail_Message(1,3,result);
						}
						for (a8 = 0; FileworksDirpath[a8]; ++a8) { }//считаем символы
...
uint32_t ModifyParameter (uint32_t Inp, uint32_t *arg){
