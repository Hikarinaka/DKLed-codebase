...
uint8_t* I2C_TargetAddress_Pointer;//указатель на переменную чтобы проще переключаться между выводом на I2C и заполнением массива текстов по кнопкам
...
uint8_t Current_Color_R;
uint8_t Current_Color_G;
uint8_t Current_Color_B;
...
extern uint8_t DS_Math_Flags; //флаги активации разных арифметических операций
//0 - скопировать значение, 1 - добавить, 2- вычесть, 3 - взять из переменной
...
//если нам не важно чтобы в процессе отправлялся именно тот символ, который нужен
		...
		switch (*DS_status_) {
			case DS_Type_command_new: //начало принятия команды
				//*DS_Param_ = 0;
				DS_Flag_Register &=~0x7F; //по умолчанию всё в HEX,  снимаем биты 1,2,4,8, 16, 32 (0x1+0x2+0x4+0x8+0x10+0x20+0x40), указывающий на расширенную команду
				DS_Math_Flags = 0;
				*DS_comm_num_=0;
				*DS_RGB_counter_ = 0;
				...
				}else if (DS_current_char == 'A'){ //сокращение "G25 QH"- вызов события (событие по кнопке, прямой номер)
					*DS_comm_num_ = 0x80;
					*DS_status_ = DS_Type_command_BUTTON_EMULATE;
			...
			case DS_Type_command_S: // ждем номер для S-команды
				...
				if (DS_a8 == 0xFF){ //если принятый символ не цифровой
					*DS_Param_ = 0;
					...
					else if (DS_current_char == 'K'){// прием цветов для палитры,дальше ждём номер цвыета скоторого начать заполнять
						*DS_status_ =  DS_Type_command_SK;
						*DS_WSpoint_counter_ = 0;
						DS_Status_backup = DS_Flag_Register;
						DS_Repeater_set = 0;
						DS_Repeat_Count = 1;
...
//					if (*DS_maxCurrPoints_ < *DS_WSpoint_counter_){*DS_maxCurrPoints_ = *DS_WSpoint_counter_;} //количество обновляемых точек берем из счетчика точек
				...
				if (DS_a8 == 0xFF){ //если принятый символ не цифровой
					...
					//	continue;//выход на новый виток
						...
						*DS_RGB_counter_ = 5;
						if ((DS_Status_backup & 0x41) && (*DS_Param_ & 0x0FFF)>1){
							*DS_Param_ -=1;
						}
						DS_Flag_Register = DS_Status_backup;
						DS_Repeater_set = 2;
...
S0P_Next_Loop_Swap_Pixel_label:
						...
						*DS_RGB_counter_ = 2;		//номер символа в точке
						continue;//выход на новый виток
					} else if(DS_current_char == 'K'){ //взять цвет из палитры
						DS_Flag_Register |= 0x1;
						*DS_RGB_counter_ = 4;
						continue;//выход на новый виток
...
S0P_Error_Unexpected_Symbol:
					...
					*DS_RGB_counter_ += 1;
					...
					if (*DS_RGB_counter_ >= 6){ //когда случилось 6 цифровых символов - распознавание и помещение в память буфера
						...
						if (DS_Flag_Register & 2){
							*DS_WSpoint_counter_ = *DS_Param_;
							*DS_Param_ = CRC_sum;
							DS_Flag_Register &= ~0x02;
						} else {
							DS_Status_backup = DS_Flag_Register;
							...
							if (DS_Flag_Register & 0x40){
								...
								if (DS_Flag_Register & 0x1){
									*DS_Param_ = WS2812_IO_FRAMEDATA_SIZE - ((*DS_Param_ & 0xFF) * 3);
									*DS_Param_ = WS2812_IO_framedata_[*DS_Param_] + (WS2812_IO_framedata_[*DS_Param_ - 1]<<8) + (WS2812_IO_framedata_[*DS_Param_ - 2]<<16);
									DS_Flag_Register &= ~0x1;
									DS_Status_backup &= ~0x1;
								}
...
//SV-------------------------------------------------------------------------------------------------------------------------------------------------
			case DS_Type_command_SK: //расшифровка цветов палитры точек для WS
				if (DS_End_Of_Command_flag){
					*DS_status_ = DS_End_Of_Command_flag;
					return DS_ANS_WS_S_DONE;
				}
				...
				if (DS_a8 == 0xFF){ //если принятый символ не цифровой
					if (*DS_RGB_counter_){
						goto SVP_Error_Unexpected_Symbol;
					} else if (DS_current_char == 'G') { //если команда на пропуск точки
						*DS_WSpoint_counter_ += 1;	//текущий принимаемый номер точки WS
SVP_Next_Loop_label:
						*DS_RGB_counter_ = 0;		//номер символа в точке
						...
						*DS_RGB_counter_ = 4;		//номер символа в точке
						...
						goto SVP_Next_Loop_label;
					} else {
SVP_Error_Unexpected_Symbol:
						Error_Message_full('S', 'K',0, 0, 1,DS_i, DS_current_char);
						return DS_ERR_UNEXPEXTED_SYMBOL; // если это не конец строки
					}
				...
				*DS_RGB_counter_ += 1;
				if (*DS_RGB_counter_ >= 6){ //когда случилось 6 цифровых символов - распознавание и помещение в память буфера
					if (DS_Flag_Register & 0x02){//определить стартовый пиксель палитры
						*DS_WSpoint_counter_ = *DS_Param_ & 0xFF;
						*DS_Param_ = CRC_sum;
						DS_Flag_Register &= ~0x02;  //сброс флага смены пикселов
					} else {//записываем цвет
						if (DS_Repeater_set & 1) {
							DS_Repeat_Count = *DS_Param_ & 0xFF;
							*DS_Param_ >>= 8;
							if (DS_Repeat_Count < 2) {goto SVP_Skip_Any_Pixel_Action_Label;}

							//DS_Repeat_Count = ((*DS_WSpoint_counter_ + DS_Repeat_Count) <= (MAX_PALLETTE_SIZE+1))? (DS_Repeat_Count - 1): (MAX_PALLETTE_SIZE + 1 - *DS_WSpoint_counter_);
							DS_Repeater_set = 0;
							DS_Repeat_Count -= 1;// поскольку функции работы с массивом цветов осуществляют один проход в любом случае, мы записываем сколько раз дополнительно повторить этот действие помимо самого первого
							// запись "*00" не осуществляет повтор, а запись ">*00" один повтор осуществит, т.к. в момент чтения ">" система ещё не знает, сколько раз ей повторять
						}
						//DS_Status_backup = DS_Flag_Register;
						Current_Color_R = *DS_Param_>>16;
						Current_Color_G = *DS_Param_>>8;
						Current_Color_B = *DS_Param_;
						do{
							if (*DS_WSpoint_counter_ >= MAX_PALLETTE_SIZE){
								*DS_status_ = DS_Type_command_new;
							//	*DS_buf_start_ = DS_i + 1;
								Error_Message_full('S', 'K',0, 0, 3,DS_i, DS_current_char);
								return DS_ERR_TOO_MANY_SYMBOLS;
							}

							WS2812_IO_framedata_[WS2812_IO_FRAMEDATA_SIZE - (*DS_WSpoint_counter_ * 3)] = Current_Color_B; //Blue component
							WS2812_IO_framedata_[WS2812_IO_FRAMEDATA_SIZE - 1 - (*DS_WSpoint_counter_ * 3)] = Current_Color_G; //Green component
							WS2812_IO_framedata_[WS2812_IO_FRAMEDATA_SIZE - 2 - (*DS_WSpoint_counter_ * 3)] = Current_Color_R; //Red component
							DS_Repeat_Count -= 1;
							*DS_WSpoint_counter_ += 1;
						} while (DS_Repeat_Count);

					}

SVP_Skip_Any_Pixel_Action_Label:
					DS_Repeat_Count = 1;
					*DS_RGB_counter_ = 0;		//номер символа в точке
					//*DS_Param_ = 0;				//значение цвета точки в RGB - временное хранилище
							...
							case 1:	//список папок/файлов в корне 0 - первая, 1 - следующая по порядку в памяти
								if (DS_current_char == 'P'){// если прием номера s-кода закончен (буква P), определяем цифру
									...
									*DS_status_ =  DS_Type_command_FILEWORKS_CONT;
									//	*DS_buf_start_ = DS_i + 1;
								//	*DS_Param_ = 0;
									*DS_RGB_counter_ = ((SD_Volume_Exists>>2)&3) - 1;
								} else {
...
Setting_Of_Variables:	//case 0x2B = M91, 0x16 = M46, M45

							//DS_Flag_Register &= ~(0xD);
							if (DS_End_Of_Command_flag){
								*DS_status_ = DS_End_Of_Command_flag;
								return DS_ANS_MULTI_SET_PARAM; //назначаем параметр 0
...
Finish_Command_On_The_Spot_Without_Reading_All_Line_Label:
					...
					//	case 0x97:  //M97 - активация/деактивация кнопок M97 Q<кнопка> P<0-выкл, 1-вкл.>
						...
						/*case 0x97:  //M97 - активация/деактивация кнопок M97 Q<кнопка> P<0-выкл, 1-вкл.>
							...
							break;*/
						case 0x88: //1000 1000
							*DS_comm_num_ |= 0x10; // 0x98 = 1001 1000
...
//команды управления usart
			case DS_Type_command_U:
				/* спользование ифов занимает меньше места чем свитч-кейс
				switch (DS_current_char) {
				case '0': //выбор порта вывода
				case '1':
				case '2':
					*DS_RGB_counter_ |= (DS_current_char - '0');
					*DS_status_ = DS_Type_command_U;
					break;
				//case 'P': //обавить текст - пресет
				//	I2C_TargetAddress_Pointer = &TextToSend_TargetAddress;
				//	*DS_RGB_counter_ = BUTTONS_TEXTS_OUT_LABEL;
				//	*DS_status_ =  DS_Type_command_I2C;
				//	break;
				case 'C': //управление I2C
					I2C_TargetAddress_Pointer = &I2C_TargetAddress;
					*DS_RGB_counter_ = I2C_OUT_LABEL;
					*DS_status_ =  DS_Type_command_I2C;
					break;
				case 'I': //присвоение ID
					*DS_Param_ = EMPTY_PERSONAL_ID;
					*DS_RGB_counter_ = 0;  // сброс переменных для распознавания ID
					*DS_status_ =  DS_Type_command_UID;
					break;
				case 'S': //установка флага переключения в приём по байтам
					*DS_comm_num_ = DS_USART_SET_SHORT - DS_USART_SET_WAIT;
				case 'F': //установка флага ожидания конца передачи
					*DS_comm_num_ += DS_USART_SET_WAIT - DS_USART_SET_INPUT;
				case 'R': //включение/отключение приёма
					*DS_comm_num_ += DS_USART_SET_INPUT - DS_USART_SET_DEBUG;
				case 'D': //включить/отключить дебаггер
					*DS_comm_num_ += DS_USART_SET_DEBUG;
					*DS_status_ =  DS_Type_command_SWITCH_ONOFF;
					break;
				case 'W'://установка максимального времени приёма команды
					*DS_comm_num_ = DS_USART_SET_TIMEOUT -  DS_USART_SET_BAUD;
				case 'B': //настройка baudrate
					...
					*DS_Param_ = 0; //смысл этой переменной в каждом случае см в соответствующих ифах
					...
					if (DS_current_char == 'I'){ //присвоение ID
						...
						*DS_comm_num_ = DS_USART_SET_SHORT;
						...
						//*DS_Param_ = 0; //здесь теперь будет храниться вкл/выкл
						//*DS_buf_start_ = DS_i + 1;
					/*} else if (DS_current_char == 'P'){//обавить текст - пресет
						I2C_TargetAddress_Pointer = &TextToSend_TargetAddress;
						*DS_RGB_counter_ = BUTTONS_TEXTS_OUT_LABEL;
						*DS_status_ =  DS_Type_command_I2C;*/
...
//I2C control-------------------------------------------------------------------------------------------------------------------------------------
			case DS_Type_command_I2C:
				...
				} else if (DS_current_char == 'T'){//управление I2C - отправить текст
					//*DS_RGB_counter_ = I2C_OUT_LABEL;
					*DS_status_ =  DS_Type_command_I2C_Send_to;
					//I2C_Bytes_To_Send = 0;
					//I2C_stage = I2C_stage;
					//I2C_Bytes_To_Send = I2C_Bytes_To_Send;
				} else if (DS_current_char == 'R'){//управление I2C - прочитать
					DS_Flag_Register |=2;
					//*DS_RGB_counter_ = I2C_OUT_LABEL;
					*DS_status_ =  DS_Type_command_I2C_Send_to;
				...
				break;
//-------------------------------------------------------------------------------------------------------------------------------
			case DS_Type_command_A: //действие по кнопке заменяет собой "G25 QH"
/*				*DS_status_ =  DS_Type_command_BUTTON_EMULATE; //_GOTO_FILE;
				ButtonNomberCarrier = 0;
				*DS_Param_ = 0;				//*DS_Param_ - для хранения номера (имени) файла, в который переходить
				//	*DS_RGB_counter_ = 0;		//*DS_RGB_counter_ - сколько символов имени файла считано (разрешается не больше 8)
					...
					if (*DS_comm_num_ & 0x1) { //если младший бит =1, то записываем символ в P, иначе - в Q
						*DS_Param_ = DS_num_add_symbol(DS_a8, *DS_Param_);  //добавляем символ в P
						...
						if (*DS_comm_num_ == 0x80){//будем работать с командой по её номеру
							...
							DS_a8 = (DS_a8 - 1) & 7; //1...8 -> 0...7
							ButtonNomberCarrier |= (1<<DS_a8) & MAX_NUM_BUTTONS_ACIONS_MASK; //0...63
						} else {
							ButtonNomberCarrier = DS_a8; //поскольку сейчас MAX_NUM_BUTTONS = 16 и номер кнопки умещается в доин символ
						}
					}
...
Servo_Set_Value:
						if (*DS_RGB_counter_ >= 4){ //когда случилось 4 цифровых символов - распознавание и помещение в память буфера
							...
							Servo_Pos_Temp[*DS_WSpoint_counter_] =  ModifyParameter(Servo_Pos_Temp[*DS_WSpoint_counter_], DS_Param_, Servo_Resolution);
			...
			case DS_Type_command_USART_SPECS:
				...
				//	*DS_RGB_counter_ = 0;		//номер символа в точке
					...
					case 'V'://версия прошивки
							Message ("DKLed v0.8\0",(*DS_RGB_counter_ ));
...
//вывод данных через DBGU U2T H***\n - сама отправка
			...
			//case DS_Type_command_USART2_SEND_HEX:
				...
				break;
//-----------------------------------------------------------------------------------------------------------
//setting the address of I2C receiver UC T<address>, UC R<address>
			case DS_Type_command_I2C_Send_to:
				...
				if (DS_a8 == 0xFF){ //если принятый символ не цифровой
					...
					//I2C_stage = I2C_stage;
					//I2C_Bytes_To_Send = I2C_Bytes_To_Send;
					/* *I2C_TargetAddress_Pointer = ((uint8_t) *DS_Param_)&0x7F; //записываем адрес устройства-получателя
					if (*DS_RGB_counter_ == BUTTONS_TEXTS_OUT_LABEL){//надо произвести сдвиг массивачтобы очистить стэк подновый текст
						TextToSend_TargetAddress = (TextToSend_TargetAddress < BUTTONS_TEXTS_AMOUNT)? TextToSend_TargetAddress : BUTTONS_TEXTS_AMOUNT - 1;

					}/**/
					I2C_TargetAddress = ((uint8_t) *DS_Param_)&0x7F; //записываем адрес устройства-получателя
					if (DS_current_char == 'Q'){//будем осуществлять сложное чтение по определённому адресу из I2C по команде UC R<address> Q<byte in i2c device>
						//I2C_Bytes_To_Send = 0;
						DS_Flag_Register|=3; //на данный момент UC R<> Q<> и UC T<> Q<> ведут себя одинаково

					} else if (DS_Flag_Register & 2){//Будем читать из 2C: UC R<address> P/N<how much to read>
						*DS_Param_ = 0;
						*DS_status_ =  DS_Type_command_USART_BAUD;
						*DS_comm_num_ = DS_I2C_GET_FROM + (DS_Flag_Register&1);//простое или сложное чтение
						DS_Flag_Register &=~3;
						goto Set_Numeric_Value_To_A_Control_Variable_Label; //*DS_status_ = DS_Type_command_USART_BAUD;, но поскольку у нас уже есть символ P/H для обработки, прыгаем в нынешнем цикле
					} else {
						goto Data_Out_Trough_Ports_Format_Choice_Label;//и дальше у нас действия как при обычной отправке двнных
					}
				} else {  //если сюда дошли - добавляем свежеполученный символ в параметр и продолжаем расшифровку
					if (DS_Flag_Register&1){
						*DS_WSpoint_counter_ = DS_hex_add_symbol(DS_a8, *DS_WSpoint_counter_);
						I2C_Bytes_To_Send ++;//поскольку мы будем отправлять адрес читаемой ячейки
					}else{//получаем <address> I2C устройства
						*DS_Param_ = DS_hex_add_symbol(DS_a8, *DS_Param_);
					}
				}
				break;
...
{
	//uint32_t i32 = 0;
	if ((DS_Flag_Register & 1)){ // байт 1 поднят
		par *= 10;
//		par += c_hex;
//		return par;
	}
