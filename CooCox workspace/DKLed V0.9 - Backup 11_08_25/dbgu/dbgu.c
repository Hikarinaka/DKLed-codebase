...
if (P < 3){//вывод на UART. если P=2, то надо отправить на оба
...
} else if (P == I2C_OUT_LABEL){//вывод на I2С
...
/*} else if (P == BUTTONS_TEXTS_OUT_LABEL){//выводв короткие тексты для кнопок
	if (TextToSendArrayPointer < BUTTONS_TEXT_ARRAY_SIZE) {
		TextToSendArrayForButton[TextToSendArrayPointer] = data;
		TextToSendArrayPointer ++;

	} else {
		Message (": Not enough memory\0",DEBUG_PORT_OUT);
#if (DEBUG_PORT_OUT == 0xf)
		USB_main_COM_react();
#endif
	}/**/
