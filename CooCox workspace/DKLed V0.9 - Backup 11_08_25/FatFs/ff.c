...
)
{
	...
	// этой строчкой парсер ошибок тоже недоволен
...
)
{
	...
	*path = &p[si];						/* Return pointer to the next segment */
	...
	for (;;) {
		...
		if (c == '.' || i >= ni) {//дочитали до точки или прочитали 8 символов
			if (ni != 8 || c != '.') return FR_INVALID_NAME; //это вторая точка на пути, или уже 9й символ а точки всё нет
			i = 8; ni = 11; //перемещаемся на позицию начала ввода расширения
			b <<= 2; continue;
		}
...
/*	vol = p[0] - '0';					// Is there a drive number?
	...
	vol = 0; //у нас все пути начинаются с 0:
	p += 2; *path = p;
	...
	if (res == FR_OK) {
		...
		if (res == FR_OK) {						/* Old object is found */
			if (!djo.dir) {						/* Is root dir? */
				...
				//res = chk_mounted(&path_new, &djn.fs, 1);
				//предполагается, что новое имя находится на том же диске. Но у нас все имена начинаются с "0:", и эти символы надо отрезать
				res = follow_path(&djn, path_new+2);
