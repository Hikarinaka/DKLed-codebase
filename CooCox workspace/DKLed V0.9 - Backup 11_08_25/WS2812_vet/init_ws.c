...
//	  NVIC_InitTypeDef NVIC_InitStructure;
	  ...
	  PrescalerValue = (uint16_t) ( SystemCoreClock / 24000000 ) - 1;
	  /* Time base configuration */
	  //TIM_TimeBaseStructure.TIM_Period = Tinit_tc; // 800kHz периояд вывода битов, можно менять в процессеисполнения программы
	  //TIM_TimeBaseStructure.TIM_Period = Timer4_counter; // 800kHz период вывода битов, можно менть в процессеисполнения программы
		TIM4->ARR = Tinit_tc;
